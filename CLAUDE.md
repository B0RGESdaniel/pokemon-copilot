...
- **`font-pix` sizing (Press Start 2P)**: this font is monospace at exactly 1em per character and only renders crisply at multiples of its 8px design grid — `text-[7px]`/`text-[9px]`/etc. blur under the `-webkit-font-smoothing: none` set on `body` in `index.css`. Always use `text-[8px]`, `text-[16px]`, or `text-[24px]`; never an arbitrary in-between value. Bumping a size a rung (e.g. 8→16) roughly doubles its rendered width — check the element isn't sharing a single line with other fixed-width siblings (a badge, a button, another span) before doing it, since Press Start 2P's 1em/char width is easy to underestimate and several spots in this app (the `w-28`/`w-[138px]` battle action-button columns, the 260px save-switcher dropdown, the `w-[62px]` stat-label column) are too narrow to survive the jump to 16px. When in doubt, keep it at 8px rather than guess.
