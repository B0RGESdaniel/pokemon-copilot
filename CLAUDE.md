...
- **Pixel-font sizing**: both display fonts render under the app-wide `-webkit-font-smoothing: none` (`index.css`), so arbitrary sizes look blurry/uneven — each font has its own grid, and a size must land on it:
  - `font-pix` (Press Start 2P) is exactly 1em per character; its grid is **multiples of 8px** (`text-[8px]`, `text-[16px]`, `text-[24px]`...).
  - `font-vt` (VT323) is exactly 0.4em per character; its grid is **multiples of 5px** (`text-[10px]`, `text-[15px]`, `text-[20px]`, `text-[25px]`...) — 8px multiples do *not* work for this font (0.4×8 = 3.2, a fractional character width).
  - Never use an arbitrary in-between value (`text-[7px]`, `text-[9px]`, `text-[14px]`, `text-[19px]`, etc.) — round to the nearest rung on the relevant grid, and preserve whatever hierarchy already existed (something that was visibly bigger than its neighbors before should land on a *higher* rung, not the same one everything else got rounded to).
  - Before bumping a size up a rung, check the element isn't sharing a single line with other fixed-width siblings (a badge, a button, another span) — a rung jump can nearly double the rendered width, and several spots in this app (the `w-28`/`w-[138px]` battle action-button columns, the 260px save-switcher dropdown, the `w-[62px]` stat-label column) are too narrow to survive it. When in doubt, keep the smaller rung rather than guess.
