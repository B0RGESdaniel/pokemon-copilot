...
- **Fonts**: after trying several alternatives for body/numeric text (VT323, Geist Pixel, Silkscreen, Jersey 10, Share Tech Mono) and finding all of them worse for legibility than Press Start 2P itself, the app now uses **Press Start 2P for everything** — `--font-vt` in `index.css` is aliased to the same family as `--font-pix` rather than a distinct font. The two Tailwind classes (`font-pix`, `font-vt`) were kept as-is in components (not merged/renamed) to avoid a mechanical rename across ~15 files; treat them as equivalent when reading code.
- **Pixel-font sizing**: this font renders under the app-wide `-webkit-font-smoothing: none` (`index.css`), so arbitrary sizes look blurry/uneven. It is exactly 1em per character (true monospace); its grid is **multiples of 8px** (`text-[8px]`, `text-[16px]`, `text-[24px]`...). Never use an in-between value (`text-[7px]`, `text-[9px]`, `text-[15px]`, `text-[20px]`, etc.) — round to the nearest rung, preserving whatever hierarchy already existed (something visibly bigger than its neighbors should land on a *higher* rung, not get flattened to the same one as everything else).
  - Before bumping any size up a rung, check the element isn't sharing a single line with other fixed-width siblings (a badge, a button, another span) — a rung jump can nearly double the rendered width, and several spots in this app (the `w-28`/`w-[138px]` battle action-button columns, the 260px save-switcher dropdown, the `w-[62px]` stat-label column) are too narrow to survive it. When in doubt, keep the smaller rung rather than guess.
